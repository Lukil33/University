...
void inserimento_manuale(Studente ** database, int i, int & DIM);
...
void crea_database(int & DIM, char * argv[], Studente ** database){
    ...
    while((file>>nome>>cognome>>matricola>>media)&&(i<DIM)){
        database[i] = crea_persona(nome, cognome, matricola, media);
        i++;
    }

    inserimento_manuale(database, i, DIM);
}

void inserimento_manuale(Studente ** database, int i, int & DIM){
    char nuovo_inserimento;
    char nome[31], cognome[31];
    int matricola;
    float media;

    cout<<"Vuoi inserire degli studenti? (Y/N): ";
    cin>>nuovo_inserimento;
    while((nuovo_inserimento=='Y')||(nuovo_inserimento=='y')){
        cout<<"Nome: ";
        cin>>nome;
        cout<<"Cognome: ";
        cin>>cognome;

...
list* estrai(list* dip, list* ent, list* usc){
    if(dip != NULL){
        if(movimenti(ent, dip->code) == movimenti(usc, dip->code)){
            return new list{dip->code, estrai(dip->next, ent, usc)};
        }else{
            return estrai(dip->next, ent, usc);
        }
    }else{
        return NULL;
    }
...
int movimenti(list* l, int num){
    if(l != nullptr){
        if(l->code == num){
            return 1 + movimenti(l->next, num);
        }else{
            return movimenti(l->next, num);
        }
    }
    return 0;
}

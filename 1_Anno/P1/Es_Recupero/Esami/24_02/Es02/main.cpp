...
node* makeOperation(node* Fn, node* Sn){
    ...
    if(isFalse(Fn) || isFalse(Sn)){
        return makeFalse();
    }
    if(getVar(Fn) == getVar(Sn)){
        node* T = makeOperation(Fn->Then, Sn->Then);
        node* E = makeOperation(Fn->Else, Sn->Else);

        if(areEquivalent(T, E)){
            deleteNode(E);
            return T;
        }else{
            return makeNode(getVar(Fn), T, E);
        }
    }else if(getVar(Fn) < getVar(Sn)){

...
node* makeOperation(node* Fn, node* Sn){
    if((Fn->Then == nullptr && Fn->Else == nullptr) || (Sn->Then == nullptr && Sn->Else == nullptr)){
        if(isTrue(Fn)){
            return makeCopy(Sn);
        }else if(isTrue(Sn)){
            return makeCopy(Fn);
        }else if(isFalse(Fn) || isFalse(Sn)){
            return makeFalse();
        }
    }else{
        if(Fn->var == Sn->var){
            if(areEquivalent(makeOperation(Fn->Then, Fn->Else), makeOperation(Sn->Then, Sn->Else))){
                deleteNode(makeOperation(Sn->Then, Sn->Else));
                return makeOperation(Fn->Then, Fn->Else);
            }else{
                return makeNode(getVar(Fn), makeOperation(Fn->Then, Fn->Else), makeOperation(Sn->Then, Sn->Else));
            }
        }else if(Fn->var < Sn->var){
            return new node{getVar(Fn), makeOperation(Fn->Then, Sn), makeOperation(Fn->Else, Sn)};
        }else{
            return new node{getVar(Sn), makeOperation(Sn->Then, Fn), makeOperation(Sn->Else, Fn)};
        }
    }
}

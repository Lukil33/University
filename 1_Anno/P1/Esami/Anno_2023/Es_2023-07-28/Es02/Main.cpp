...
// implementare
void ausiliary(list*& result, float x, int N, double precision, int counter){
    if(counter <= N){
        list* created=new list{0, result};

        if(created->next != nullptr){
            created->info=pow(x, 2*counter+1)/factorial(2*counter+1)+created->next->info;
            double abs_sinhN_1=created->next->info*(created->next->info>=0)-created->next->info*(created->next->info<0);
            double abs_sinh=(created->info - created->next->info)*(created->info - created->next->info>=0)-(created->info - created->next->info)*(created->info - created->next->info<0);

            if((abs_sinh) >= (precision * abs_sinhN_1)){
                result=created;
                ausiliary(result, x, N, precision, counter+1);
            }
        }else{
            created->info=pow(x, 2*counter+1)/factorial(2*counter+1);
            result=created;
            ausiliary(result, x, N, precision, counter+1);
        }
    }
...
list* function(float x, int N, double precision){
    list* result=nullptr;
    ausiliary(result, x, N, precision, 0);
    return result;
}

...
int altezza_iter(const tree* root){
    if(root == nullptr){
        ...
        bool continua=true;
        enqueue(coda, root);
        const tree* inizio=root;
        int altezza=1;
        while(continua){
            if(empty(coda)){
                continua=false;
            }else{
                if(inizio == first(coda)){
                    if(coda->head->node->left != nullptr){
                        inizio=coda->head->node->left;
                        altezza+=1;
                    }else if(coda->head->node->right != nullptr){
                        inizio=coda->head->node->right;
                        altezza+=1;
                    }else{
                        if(coda->head->next != nullptr){
                            inizio=coda->head->next->node;
                        }else{
                            inizio=nullptr;
                        }
                    }
                ...
                if(coda->head->node->left != nullptr){
                    enqueue(coda, coda->head->node->left);
                }
                if(coda->head->node->right != nullptr){
                    enqueue(coda, coda->head->node->right);
                }
                dequeue(coda);
            } 
        }

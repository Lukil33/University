...
void ausiliary(int num, int den, int* res, int res_maxdim, int counter){
    if(den != 1){
        res[counter]=num/den;
        int new_num=den/gcd(den, num%den);
        int new_den=(num%den)/gcd(den, num%den);
        ausiliary(new_num, new_den, res, res_maxdim, counter+1);
    }else{
        res[counter]=num/den;
    }
}
...
void compute_continued_fraction_elements(int num, int den, int* res, int res_maxdim){
    ausiliary(num, den, res, res_maxdim, 0);
}
